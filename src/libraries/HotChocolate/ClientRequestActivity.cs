...
namespace Thor.HotChocolate
{
        ...
        : IActivity
    {
        ...
        public static ClientRequestActivity Create(string method, Uri uri)
        {
            ...
            {
                RequestActivityEventSource.Log.Send(context._activityId, method, uri.ToString());
            }
            else
            {
                RequestActivityEventSource.Log.BeginTransfer(context._relatedActivityId);
                RequestActivityEventSource.Log.Send(context._activityId, method, uri.ToString());
                RequestActivityEventSource.Log.EndTransfer(context._activityId, context._relatedActivityId);
            }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposed)
            {
                if (disposing)
                {
                    RequestActivityEventSource.Log.Receive(_activityId, _userId ?? Guid.Empty, _statusCode ?? 0);
