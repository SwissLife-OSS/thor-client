...
namespace Thor.HotChocolate
{
        ...
        : IActivity
    {
        ...
        public static ServerRequestActivity Create(string method, Uri uri, Guid? relatedActivityId)
        {
            ...
            {
                RequestActivityEventSource.Log.OuterActivityNotAllowed(context.Id);
            }

            if (relatedActivityId != null && relatedActivityId != Guid.Empty)
            {
                RequestActivityEventSource.Log.BeginTransfer(relatedActivityId.Value);
                RequestActivityEventSource.Log.Start(context._activityId, method, uri.ToString());
                RequestActivityEventSource.Log.EndTransfer(context._activityId, relatedActivityId.Value);
            }
            else
            {
                RequestActivityEventSource.Log.Start(context._activityId, method, uri.ToString());
        ...
        public static ServerRequestActivity Create(HttpRequest request, Guid? relatedActivityId)
        {
            ...
            {
                RequestActivityEventSource.Log.BeginTransfer(relatedActivityId.Value);
                RequestActivityEventSource.Log.Start(context._activityId, request);
                RequestActivityEventSource.Log.EndTransfer(context._activityId, relatedActivityId.Value);
            }
            else
            {
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposed)
            {
                if (disposing)
                {
                    ...
                    {
                        RequestActivityEventSource.Log.Stop(_activityId, _userId ?? Guid.Empty, _statusCode ?? 0);
                    }
                    else
                    {
                        RequestActivityEventSource.Log.Stop(_activityId, _httpResponse);
                    }
